...
function update() {
    ...
    ghosts.forEach(ghost => {
        ...
        // Оновлення цілей привидів
        if (Math.random() < GHOST_UPDATE_TARGET_FREQ) {
            if (ghost.color === 'red') {
                ghost.targetX = player.x;
                ghost.targetY = player.y;
            } else if (ghost.color === 'pink') {
                ghost.targetX = player.x + player.dx * 4 / TILE_SIZE;
                ghost.targetY = player.y + player.dy * 4 / TILE_SIZE;
            } else {
        ...
        if (validDirs.length > 0 && (isCloseToAligned(ghost.x) && isCloseToAligned(ghost.y) || ghost.stuckCounter > 5)) {
            ...
            validDirs.forEach(dir => {
                ...
                const futureX = nextX + dir.dx / TILE_SIZE;
                const futureY = nextY + dir.dy / TILE_SIZE;
                const futureValid = canMove(futureX, futureY);
                const deadlockPenalty = futureValid ? 0 : 50;

                const reversePenalty = (avoidReverse && dir.dx === -ghost.dx && dir.dy === -ghost.dy) ? 100 : 0;
                const stuckPenalty = (ghost.stuckCounter > 10 && dir.dx === ghost.dx && dir.dy === ghost.dy) ? 200 : 0;

                const totalScore = dist + deadlockPenalty + reversePenalty + stuckPenalty;
            ...
            const randomChance = ghost.stuckCounter > 10 ? 0.7 : (ghost.color === 'lime' ? 0.5 : 0.2);
        ...
        if (isMoving) {
            ghost.x = nextX;
            ghost.y = nextY;
        } else if (ghost.stuckCounter > 15 && validDirs.length > 0) {
